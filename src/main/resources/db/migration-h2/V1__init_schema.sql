-- V1__init_schema.sql  (H2 override for dev profile)
-- H2 does not support CREATE EXTENSION or CREATE TYPE ... AS ENUM.
-- Enums are stored as VARCHAR. Everything else is identical.
...
CREATE TABLE vehicles (
    ...
    model               TEXT,
    total_weight_kg     NUMERIC(8,0),
    payload_kg          NUMERIC(8,0),
    vehicle_length_mm   NUMERIC(7,0),
    vehicle_width_mm    NUMERIC(7,0),
    vehicle_height_mm   NUMERIC(7,0),
    cargo_length_mm     NUMERIC(7,0),
    cargo_width_mm      NUMERIC(7,0),
    cargo_height_mm     NUMERIC(7,0),
...
CREATE TABLE user_accounts (
    ...
    CONSTRAINT fk_user_accounts_emp  FOREIGN KEY (festival_year, employee_id)
        REFERENCES employees (festival_year, employee_id) ON DELETE CASCADE
);
...
CREATE TABLE tasks (
    ...
    CONSTRAINT fk_tasks_vehicle FOREIGN KEY (festival_year, vehicle_id)
        REFERENCES vehicles (festival_year, vehicle_id)
);

CREATE INDEX idx_tasks_day     ON tasks (festival_year, CAST(start_ts AS DATE));
